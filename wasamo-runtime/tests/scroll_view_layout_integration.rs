...
fn scroll_path_fixture_layouts_and_scrolls_through_visual_tree() {
    ...
    run_on_owning_runtime_thread_or_skip("ScrollView scroll-path integration test", move || {
...
fn scroll_path_fixture_r2_three_level_visual_nesting_root_relative_math() {
    // Marshalled onto the runtime-owning thread (Observation 5 step 1): the
    // Compositor is created and used on one thread. See tests/common/mod.rs.
    run_on_owning_runtime_thread_or_skip("ScrollView R2 three-level nesting test", move || {
...
fn scroll_path_vstack_root_fixture_pins_window_root_fill_override() {
    ...
    run_on_owning_runtime_thread_or_skip(
        "VStack-rooted ScrollView scroll-path integration test",
        move || {
