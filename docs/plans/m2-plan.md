...
- [ ] **M2-Phase 6 — `.ui → runtime` lowering**
  ...
  - **Implementation scope (this phase, settled by the proposed ADR):**
    ...
    - [ ] **DD-M2-P6-005: `wasamo_load_ui` C ABI + error infrastructure.** In `wasamo-runtime/src/abi.rs` and `bindings/c/wasamo.h`:
      ...
      - **Verification:** build + `dumpbin /exports` confirms new symbols; pure-logic test exercising `WASAMO_ERR_WRONG_THREAD` (spawn thread, call ABI, assert error); resource-resolution dispatch tested for both path and embedded blob; `cargo build --workspace` produces no `divergence_diagnostics is never used` warning.
