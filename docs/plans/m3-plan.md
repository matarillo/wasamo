...
questions become a phase ADR at pre-doc time, per
...
from this menu and states which it uses in its phase ADR / progress
file:
...
1. **ADR Accepted.** The phase ADR (`docs/decisions/m3-phase-N-*.md`)
...
1. **Every acceptance criterion A1–A12 is discharged**, with the
   discharge recorded in the corresponding phase ADR and the plan's
...
2. **Phase 8 outputs all three deliverables**: A10 (Button
   ...
   a CHANGELOG entry.
3. **CHANGELOG entry** for M3 lands, linking each phase ADR and
   the public-draft anchor in `docs/dsl_spec.md`.
4. **Per-phase spec sync (A11) is auditable**: every phase ADR
...
- **Iteration grammar surfacing `TypedValue` pressure.** Phase 7
  ...
  collection element types). Mitigation: phase ADR opens this as
...
| M3-Phase 2 — Box layout primitive | in progress | [m3-phase-2-progress.md](progress/m3-phase-2-progress.md) | [m3-phase-2-box-layout.md](../decisions/m3-phase-2-box-layout.md) | ADR Accepted 2026-05-20; execution opened 2026-05-20 |
