...
This vision ADR resolves the governance drift flagged in
...
Phase 3 all ran as phase ADRs without invoking any RFC process, and
...
1. **Early stages (M1-M2).** BDFL, phase ADRs.
...
- That `docs/rfcs/` is set up and routable. It is not, and Phase 1
  through Phase 3 of M3 have all run successfully on phase ADRs
  without it.

Two resolution paths exist:

- **Path A (chosen).** Revise VISION to defer RFC adoption to
  post-1.0. The pre-1.0 phase remains BDFL with phase / vision ADRs;
...
- **Path B (rejected).** Stand up `docs/rfcs/` now and re-route
  ...
  through Phase 3 already ran on phase ADRs and re-routing
...
phase ADRs, `docs/rfcs/` absent) confirms the schedule never
matched reality.
...
Pre-1.0 (M1 through M6) remains BDFL with phase ADRs and vision
...
acceptance rule are deferred to a post-1.0 vision ADR. If
...
superseded with a new vision ADR pulling the transition forward
...
| m3-phase-3-wrap-panel.md §Governance note | Add a closing line pointing at this vision ADR as the resolution; do not rewrite the historical note itself (per supersede rule) | Resolves the governance gap the Phase 3 ADR flagged |

## Out of scope

- Defining the RFC template, lifecycle, or acceptance rule. Those
  are post-1.0 work and belong in a separate vision ADR when the
