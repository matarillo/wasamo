...
| A `for` member carries a collection read resolving to a declared collection-typed state, a **single-widget body**, and well-formed binders (non-empty, distinct, not colliding with a state name); a `for` appears only under VStack / HStack / WrapPanel / ZStack (not ScrollView / Box / Grid, not at component level); no `for` member appears at any depth inside a `for` body (M3-Phase 7 baseline; handler members inside a `for` body are admitted by §4.19, M4-Phase 2) | Yes | `WASAMO_ERR_IR_MALFORMED` |
| Collection assignment: `=` only on a collection-typed LHS; the RHS is a single self-receiver tail-edit form or a static list literal with matching element types; loop-local reads (`item-read` / `index-read`) appear only in binding positions or handler expressions inside the owning `for` body (M3-Phase 7 baseline; M4-Phase 2 handler admission) | Yes | `WASAMO_ERR_IR_MALFORMED` |
...
| 1.22    | 2026-08-11 | Factual correction after the M4-Phase 2 implementation sync: §8.11 no longer repeats the superseded M3-Phase 7 prohibition on handler members inside a `for` body, and its loop-local-read row now includes the handler-expression position admitted by §4.19 and the landed loader. This correction adds no M4-Phase 3 predicate or conditional surface, changes no grammar or IR form, and makes no ABI change. |
