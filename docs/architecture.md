...
**M3-Phase 8 `ToggleButton` and the writer seam (implementation-synced).**
...
landed M3-Phase 8 implementation in T8 and is implementation-synced as of
the Phase 8 close (Moment 2).
...
- The Visual carrying the surface brush has the exact `dip × s` device
  ...
  alignment may offset it. **The replacement mechanism is not yet an
  accepted contract.** [DD-M4-P1-006](../process/milestone-4/phase-1/decisions/dd-m4-p1-006-surface-brush-mapping-is-set-not-inherited.md)
  (`Status: Proposed`) proposes stretch `None` with horizontal and
  vertical alignment ratios of `0`. That candidate keeps unit scale
...
proposed mapping, storage beyond that extent is clipped rather than
