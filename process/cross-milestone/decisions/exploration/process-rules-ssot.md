...
resolved: 2026-05-26
related-adrs:
  - process/cross-milestone/decisions/doc-system.md
  - process/cross-milestone/decisions/process-rule-ssot.md
related-notes:
  - process/cross-milestone/decisions/exploration/doc-system-redesign-note.md
---

# プロセス記述の SSOT 化

> **Resolved 2026-05-26.** 本ノートで議論した Q1〜Q6 は
> [process/cross-milestone/decisions/process-rule-ssot.md](../process-rule-ssot.md)
> (DD-V-019..021) として確定済み。要約：
>
> - **Q1, Q2 → DD-V-019.** 4 つの SSOT に領域別分散：`CLAUDE.md`（強制
>   ルール）、`process/README.md`（構造規約）、`process/procedures/retrospectives.md`
>   （retro 手順）、`process/cross-milestone/`（vision decision record）。
> - **Q3 → DD-V-019 内「Forward-link discipline」.** デフォルトはリンク
>   のみ、文脈引用は SSOT の文言を verbatim でコピー。
> - **Q4 → DD-V-020.** 軽微な改訂は直接 SSOT 編集、構造的変更は
>   vision decision record を立ててから SSOT 反映。
> - **Q5.** `retrospectives.md` を `docs/notes/` → `process/` へ昇格済み。
>   今後の散在記述は「触ったとき同期」で incremental に整理。
> - **Q6 → DD-V-021.** phase-end の verification 最終 mapping は
>   `retrospectives/phase-end.md` の `## Phase-End Gate` セクションに置く。
>   per-task evidence は `implementation/evidence/` と
>   `implementation/log.md` に。
>
> 本ファイルは経緯記録として残す（議論の出発点と却下された選択肢を含む）。
...
[doc-system-redesign-note.md](./doc-system-redesign-note.md) は VISION / ROADMAP /
...
| `process/README.md` | フォルダ構造、plan/ADR の lifecycle、各種規約 | process/ ディレクトリ規約（旧 plans/README + decisions/README を統合） |
| `docs/notes/README.md` | notes/ の対象範囲、言語、lifecycle | notes/ ディレクトリ規約 |
| `process/cross-milestone/decisions/doc-system.md` | doc system 自体の vision decision record（DD-V-010..016） | "Why" を記録する vision decision record |
...
- **(c) "活きたルール"（毎回適用）と "経過的記録"（vision decision record、過去の
...
- **(a) 直接 SSOT を編集してコミット**（現在の運用に近い。今回の `b11688b`
  がそう）。
- **(b) vision decision record を立てて昇格させる**（doc-system-redesign の
  DD-V-010..016 がそうだった）。
...
- **短期の execution log。** 実装中の作業台、task list の変化、green
  workspace の記録。`process/README.md` は phase progress file を
...
Q1〜Q6 の解決は
[process/cross-milestone/decisions/process-rule-ssot.md](../process-rule-ssot.md)
DD-V-019..021 に確定。本ノート冒頭の Resolved サマリ参照。
