...
- **Re-smoke green:**
  - Owner が 2026-05-25 に rebuilt `gallery-rust.exe` で再 smoke を
    ...
    smoke evidence は
    [docs/references/m3-phase-4/](../implementation/evidence/)
...
- **Phase 4 仕様内 `scroll_y` drift の visible 表れ.**
  ...
  scope ([m3-phase-4-progress.md:18-22](../implementation/plan.md#t6--owner-manual-gui-smoke-and-any-visible-correctness-fix))
...
4. **PO に相談すべき設計判断・トレードオフ:** **あり (T6 内で
   ...
   - smoke evidence の `docs/references/m3-phase-4/` への移動可否:
     ...
     ([t9-step-end-retrospective.md](../../phase-3/retrospectives/t9.md))
...
7. **既存 ADR の Proposed 項目の新規追加、または Proposed → Accepted
   への昇格:** **なし**
   - Phase 4 ADR (`process/milestone-3/phase-4/decisions/preamble.md`) 内
...
4. **M4 handoff item:**
   - **`scroll_y` Signal drift**。Phase 4 では `arrange_scroll_view`
     ...
     drift は解消。Phase 4 ADR ([m3-phase-4-progress.md:18-22](../implementation/plan.md#t6--owner-manual-gui-smoke-and-any-visible-correctness-fix))
