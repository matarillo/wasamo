...
| R-6 | **New C / Zig hosts + CI steps fail on CI, not locally** (build ordering: `wasamoc.exe` must exist before CMake / `zig build`; AGENTS.md §Build ordering). | T6 mirrors the proven `counter-c` / `counter-zig` CI steps (which already encode the ordering); local clean-order rehearsal before push. |
