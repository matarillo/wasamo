...
directions and were approved at the `Status: Proposed` → `Accepted`
...
The Phase 6 ADR carries six framing-slate DDs (framing DD slate → ADR
numbering 1:1), plus **DD-007 and DD-008 added mid-phase** (DD-007 by the
T4 review, DD-008 by the T7 review):

| DD | Title | Decision summary |
|---|---|---|
...
| [DD-M3-P6-003](./dd-m3-p6-003-conditional-rendering-grammar-surface.md) | Conditional rendering author-facing grammar surface | **Approach 2**; **`if <bool-expr> { <widget-child> }` block form** (family-extensible to `else`/`switch`/`for`); **conditional body = a single widget child** (`widget_decl` only — **nested `if` directly in the body deferred** with the family extension, reached meanwhile by wrapping; no property/bind/handler/state/track-list, no multiple children — a multi-widget or nested-conditional body wraps in a container); condition = the narrow bool-expr already accepted by `Button.enabled` (a `bool` state identifier or `BOOL_LIT`; operators deferred). Rejected at `wasamoc check`: a non-bool / mis-placed `if` (including a **component-level `if`** that would gate/multiply the single root — admitted only inside a widget body), a non-widget, nested-conditional, or multi-child body |
| [DD-M3-P6-004](./dd-m3-p6-004-conditional-ir-and-runtime-present-absent.md) | Conditional IR representation + runtime present/absent | **Member-level structural IR** — `children: Vec<IrMember = Widget \| ControlFlow>` with a branch list so `else`/`switch`/`for` are same-family variants; IR-only (no runtime widget); Phase 6 ships the single-branch `ControlFlowNode::If` with a **single `Widget`-child `body`** (length-1, `Widget(_)`-only — a nested `ControlFlow(_)` body deferred — enforced at lowering/loader), so present/absent is one **`insert_child`/`remove_child`** (single-child granularity; the child-range form is deferred to `for`). Runtime fills **`BindingTarget::ConditionalSubtree { parent, declared_member_index }`** — the materialised insertion index is **recomputed from declared order + live-presence** at each mutation, not cached. A **Phase 6 textual IR example** (`.ui → textual IR → loaded IR`, single-branch / single-child) is pinned in the DD §Spec content seed. **Phase 6 = full destroy+rebuild**; absent=fresh-on-return is **normative author-visible semantics**, future retention is **opt-in (keyed)** so the default never breaks. **An IR-schema change.** |
| [DD-M3-P6-005](./dd-m3-p6-005-conditional-effect-lifecycle-and-drain-contract.md) | Conditional effect lifecycle + reactive-drain proof contract | **(a)** absent subtree's Effects are **disposed** via the existing structural teardown; re-present **recreates** fresh widgets + Effects (no paused-effect state). **(b)** the M3-Phase 1 synchronous non-batched drain contract (item 4) is **preserved** — toggle-then-observe holds; newly-created subtree Effects run before quiescence **within the existing `MUTATION_CAP`**, and a cap-overflowing insertion uses the existing divergence path (documented backstop, not silent staleness). **(c)** structural mutations ride the **existing drain ordering**; multiple sibling / wrapped-descendant conditionals are kept observable by the **quiescent child-order invariant** (present conditionals settle into declared document order, drain-order-independent). Reactive-drain items 1–3 are **carried forward**, re-evaluated when the `for`/multi-conditional family reveals requirements |
...
| [DD-M3-P6-007](./dd-m3-p6-007-scrollview-conditional-content-policy.md) | ScrollView conditional-content policy (mid-phase addition, T4 review) | **(a) reject conditional-only content**: ScrollView rejects a *direct* conditional member because the exactly-one-content-child contract (DD-M3-P4-001) cannot absorb a dynamically-zero member; the conditional wraps inside the single content widget. Symmetric with the `Cell` cardinality rejection (DD-M3-P5-001); `Box` (at-most-one, DD-M3-P2-001) tolerates a lone conditional. The conditionally-empty **(b)** direction is **not** rejected — deferred pending the DSL's conditional-content model choice (imperative emission vs. optional typed vs. explicit empty / fragment). Code is the T4-follow-up interim; spec sync is one §4.11 sentence + a §4.14 diagnostics row |
| [DD-M3-P6-008](./dd-m3-p6-008-component-root-window-attribute-boundary.md) | Component-root window-attribute / widget-attribute boundary (mid-phase addition, T7 review) | **(A2a) selected, in-phase Phase 6 implementation.** Give `IrComponent` a host-owned attribute surface (`host_props` / `host_bindings`, generic-list; `base: String` kept, **no `inherits` semantics**) so component-level window attributes (`title` / `backdrop` / `theme`) stop being spliced onto the **content root**, plus a **host-general attribute catalog holding only the Window entry** this phase (the gate is the *catalog*, not the base name — an uncatalogued host attr is rejected regardless of base; an empty host surface on a non-Window base is not). Resolves the dual-gate divergence (`wasamoc check` accepts pre-splice / runtime rejects post-splice on a strict ZStack root) at the **root cause** — the undifferentiated component-member namespace — rather than reconciling the gates ((D), the fallback-not-taken) or accepting-all ((C), rejected). An **IR-schema / textual-IR migration** (schema/IR-migration review tier). Host bindings ship as a structural surface the catalog admits **none** of yet (dynamic title deferred, FD-D). End-state base-type system **(B) owned by M4**; the canonical invariant A2a preserves for M4 is the **host-attribute / content-root separation**, not the field name. Spec sync folds at **T9 Moment 2** (DD Implementation handoff step 9) |

## Cross-DD decision dependencies

Most DDs decide a self-contained question, but three decisions **couple
across DDs**: choosing the primary option constrains what a dependent DD
must do. This table is an **index / map** (where to read), not the
argument — the named DD carries the Options / Comparison / Recommendation.
Each coupling has one **primary** DD that owns the choice; the dependent
DDs carry the *consequence* of that choice. **Note — this table is
retained as deliberation-time information.** It records the cross-DD
couplings *as weighed during decision-making*, including the alternative
bundles that were **not** adopted (e.g. `B2 → IG-2`), because it is the
only phase-level coupling map. The accepted path is the adopted bundle;
the binding alternatives themselves live in the owning DDs, not here.
...
slice of these bundles locally; this table is the single phase-level
map across them.
...
1. **`wasamoc check` compile-time evidence (host-independent).**
   ...
   - **Conditional condition rejection** — a non-bool condition
     ...
     shape (DD-M3-P6-003).
   - **Conditional body rejection** — an `if` body containing a
     **non-structural member** (a property / bind / handler / `state` /
     track-list directly in the body, e.g. `if open { fill: red }`), a
     **nested `if`** directly in the body (`if a { if b { … } }`), or
     **more than one child** (`if open { Box{} Text{} }`) surfaces a
     `wasamoc check` diagnostic, and the loader re-checks it as
     `WASAMO_ERR_IR_MALFORMED`: Phase 6 admits exactly **one widget
     child** in an `if` body (DD-M3-P6-003 / DD-M3-P6-004). A
     multi-widget or nested conditional is authored by wrapping (`if open {
     VStack { … } }`).
   - **Conditional placement rejection** — an `if` block in a
     position where members are not admitted (per the grammar of
     DD-M3-P6-003) surfaces a diagnostic. This **includes a
     component-level `if`** that would gate / multiply the single root
     (`IrComponent.root`): although a component body is grammatically
     `member*`, DD-M3-P6-003 admits `if` only **inside a widget body**
     and rejects a component-level `if` at `wasamoc check`, because a
     conditional root has no parent slot to insert/remove into
     (DD-M3-P6-004's `BindingTarget::ConditionalSubtree { parent,
     slot }` requires a parent). This rejection is a required test.
...
3. **Lowering / IR-roundtrip / loader-invariant evidence (host-independent).**
   ...
   - **Control-flow member roundtrip** — emit → load of an `if`
     construct (per the §Spec content seed textual-IR shape) preserves
     the branch condition and the single-child body; the control-flow
     member materialises no runtime widget (DD-M3-P6-004).
   - **Loader rejection** — a control-flow member with a non-bool /
     unresolved condition, more than one branch (until `else`), or a
     body that is empty / multi-child / non-structural / a nested
     control-flow member (Phase 6 single-widget-child rule), and a
...
4. **Windows-runtime integration evidence (mock-free, CI-gated,
   ...
   - **Conditional toggle insert/remove** — a live `WidgetNode` tree
     ...
     **inserts / removes** the subtree (and its Visuals) from the
     ...
     and two-sibling-conditional cases (the latter **including a
     preceding-conditional removal while both are present**, so the
     removal-index shift is exercised) — and the absent subtree's
     Effects **and** widget-pointer registry entries are disposed via
     `widget_destroy` / recreated on return (DD-M3-P6-005). A
     **re-evaluation-to-same-state** case (true→true / false→false)
     asserts the condition Effect is a **no-op** when presence is
     unchanged — no duplicate insertion, no spurious removal
     (DD-M3-P6-004 minimal-runtime-state idempotency).
...
1. **`else` / `else if` / `switch`.** The member-level control-flow IR
   ...
   The same extension widens the `if` **body** to admit a nested
   `conditional_member` (deferred this phase: a single `widget_decl` is
   admitted, and the `body: Vec<IrMember>` type already admits
   `ControlFlow(_)`), so nested control flow lands additively — a
   body-admission relaxation plus the 0/1-materialised-child grain
   re-statement, **no `IrMember` shape change**.
...
3. **Expression-grammar extension** (`!ready`, comparison, logical
   ...
   once), not a condition-only pocket; the control-flow member and
   runtime seam are unaffected.
...
- **Nested control flow directly in an `if` body** (`if a { if b { … }
  }`, no intervening widget) — Phase 6 admits a single `widget_decl`
  child (B1); a bare nested `if` is deferred with the family extension
  and reached meanwhile by wrapping the inner `if` in a widget
  (DD-M3-P6-003 / DD-M3-P6-004 forward-compat). **This defers the
  *immediate branch body* only, not nested-conditional semantics as a
  whole.** The owner-facing three-way boundary (DD-M3-P6-003
  §Conditional body shape Recommendation): (i) branch body admission is
  `conditional_body ::= widget_decl` only; (ii) **deferred** = a bare
  nested `if` *as the immediate body* (`if a { if b { … } }`); (iii)
  **in scope** = **sibling** conditionals and **descendant** conditionals
  nested inside the admitted widget subtree (`if a { VStack { if b { … }
  } }`) — their present/absent, quiescent child-order invariant
  (DD-M3-P6-004), and effect-lifecycle / SM-1 ordering (DD-M3-P6-005) are
...
| Date | Change |
|---|---|
| 2026-06-08 | **DD-M3-P6-008** (component-root window-attribute / widget-attribute boundary) added mid-phase — surfaced by the T7 review (the first ZStack-root example made the latent splice-onto-root model fail at the strict ZStack validator) — and accepted **(A2a)** after a multi-pass design-decision review: give the component a host-owned attribute surface (`host_props` / `host_bindings`) + a host-general catalog (Window entry only, `base: String` kept), scoped to **in-phase Phase 6 implementation** as an IR-schema / textual-IR migration. (D) the explicit fallback-not-taken; (C) rejected. §Decisions index gains an eighth row (mid-phase addition, outside the original 1:1 framing slate); living-spec sync folds at T9 Moment 2. |
| 2026-06-04 | **DD-M3-P6-007** (ScrollView conditional-content policy) added mid-phase — surfaced by the T4 review semantic-migration audit — and accepted **(a)** after a multi-pass design-decision review (strategic / recommendation-choice / implementation-readiness): ScrollView rejects a direct conditional member; the conditionally-empty **(b)** direction is deferred, not rejected. §Decisions index gains a seventh row (mid-phase addition, outside the original 1:1 framing slate). |
| 2026-06-02 | Owner agreed to all six DDs' recommended options. Preamble §Decisions restructured to record the accepted decisions only; weighed alternatives retained in the owning DDs and in the deliberation-time §Cross-DD map. |
| 2026-06-01 | Review revisions folded: DD structure refactoring → Strategic / owner-alignment review → recommendation-choice review → implementation-readiness review. Status: Proposed. |
