...
**位置づけ — これは強制ゲートではない。**
本ひな形は **guideline**（おすすめ手順）であり、守るかどうかは自由。

### task-end retroepectve

```
---
phase: <マイルストーン・フェーズの表記>
task: <タスク番号>
title: 
date: 
scope: task-end
...
<ダブルループの観点で、目標・前提・事前計画を設定したり見直したりする観測事実についてのみ記述。シングルループの内容は削除>

### 自己分析（どの層が外れたか）
<ダブルループの観点で、目標・前提・事前計画の自己分析についてのみ記述。シングルループの内容は削除>

### 計画仮説・前提の問い直し（オーナーと）
<ダブルループの観点で、目標・前提・事前計画の問い直しについてのみ記述。シングルループの内容は削除>

### 反証可能な是正テスト
<ダブルループの観点で、目標・前提・事前計画を設定したり見直したりする方法の是正についてのみ記述。シングルループの内容は削除>

### 後続タスク / オーナーへ共有すること
<ダブルループの観点で、目標・前提・事前計画を設定したり見直したりする方法とその是正の共有事項についてのみ記述。シングルループの内容は削除>

## マージゲート
<`process\procedures\implementation-gates.md` の Review lane も併せて記載>
```
