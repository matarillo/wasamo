...
| `TypedValue` + structured data (M-expr2b/3: computed color / dimension, `item.filename`) | unknown — the `abi_spec` impact check is due during M4 and is owned by [M4-Phase 7](./milestone-4/plan.md), where the host value boundary is designed. That assigns the decision point; it does **not** assume Phase 7 is M4's only or last ABI-bearing phase. If the verdict is still unknown later, [DD-V-028](./cross-milestone/decisions/pre-1.0-candidate-pool.md)'s last-planning-before-M6 and M6-planning backstops apply | dedicated pre-1.0 slot (M4–M5) **if the ABI verdict and product value justify it**; M5 planning is the last routine §1.1 disposition before M6, not an automatic commitment to create a slot. Two further pool rows consume the verdict — reactive `fill` gravitates here by name, and background color on themed widgets reaches M5 theming through it | [expression-language-roadmap](../docs/notes/expression-language-roadmap.md), [typed-value-evaluator](../docs/notes/typed-value-evaluator.md) |
...
| Button keyboard activation — a focused `Button` raising `clicked` from a key press (`Space` / `Enter`), which the runtime has never done. `docs/dsl_spec.md` 1.21 corrected the public text to say that Button keyboard activation is **not** part of the current widget surface. The remaining backward-compatibility issue is `Enter`: it is a valid authored `key-down("Enter")` key today and is not in §4.19's keys-the-runtime-keeps table, so making a focused Button consume it for activation would stop an existing handler from firing. If the Button family adopts `Enter` activation, that author-visible contract should therefore be settled before the 1.0 compatibility commitment despite `ABI-bearing: no`. `Space` is not a recognised `key-down` name today and is not evidence of the same existing-program break. Declining keyboard activation requires no further spec correction; the current spec already states that outcome. **Owner direction (2026-08-08): better decided alongside the other keyboard-operable controls** — a DropDown, CheckBox or Radio each activate from the keyboard too, and one activation contract for the family beats retrofitting Button's | no — dispatch and presentation only; `clicked` already reaches hosts through `wasamo_signal_connect` | **none claimed** (owner, 2026-08-08); revisit with the M5 widget set's keyboard-operable controls | [M4-Phase 2 T8 close gate CF-T8-1](./milestone-4/phase-2/implementation/log.md) (2026-08-08), owner-added |
| Prefix-set final form — the shape the closed set of "where to look this name up" prefixes carries into 1.0, in two parts. **(1) Membership:** whether the expression-side set is **emptied**, retiring the `root.` prefix so state is reached by bare name only. **(2) Spelling:** whether members must begin with a reserved symbol (`^slot` / `^root` / `^host`; `^` leads and `@` is the alternative, while `#` is ruled out because it already spells a color literal). **Deciding whether to change at all comes before choosing a symbol.** Both are freeze-relevant. A bare new prefix has to be claimed out of the identifier namespace, so adding `host` would break — or, under today's last-segment-wins resolution, silently change the meaning of — any `.ui` holding a state named `host`; a symbol form cannot collide, which keeps the set **extensible after the freeze**. And if M4-Phase 7 marks the host boundary on declarations rather than with a prefix, `root` is left as a prefix with no work to do — an outcome that produces no artifact of its own, which is why the question needs a row a planning pass must dispose of rather than prose someone has to remember. Migration is mechanical, diagnosable and moves no meaning (`slot.` 22 in `gallery.ui` plus 112 in `dsl_spec.md`; a `root.` prefix 26 times across four `.ui`), so it can follow the M4-Phase 3 dot-semantics rule — the dot's left side is a value, the exceptions are a closed prefix set — rather than ride with it | no — author-surface spelling only; a dotted prefix is already discarded at lowering (`root.count` emits `(prop-read count)`) and placement lowers to `child { placement … }` records, so neither the textual IR nor the C ABI carries the spelling | **M4-Phase 7 as the natural decision point for both parts** — the host state boundary is where a new prefix would first be claimed, where a symbol form first pays for itself, and where the answer to "does `root` still have work to do" falls out. Backstop: settled before the 1.0 compatibility commitment, since neither membership nor spelling can move after the M6 freeze | M4-Phase 3 pre-doc discussion (2026-08-12, owner-added); depends on the M4-Phase 3 dot-semantics record, which is not yet filed |
| Focus-annotation surface — revisit whether a container may be **both** a focus group and a modal scope. [dsl_spec.md §4.19](../docs/dsl_spec.md) spells them as two independent boolean attributes (DD-M4-P2-005 option A1) so the combination remains expressible, but the current `focus_core::FocusRole` holds one role per node: `modal-scope` wins, `focus-group` is inert, and `wasamoc` warns. Two current candidate dispositions can both affect accepted `.ui`: collapsing the booleans into one enumerated attribute changes spelling, while implementing a combined role changes the runtime meaning of a source shape that is accepted today with a warning. **Current scheduling hypothesis:** if Wasamo intends to support either changed contract, settle it before the 1.0 compatibility commitment; otherwise 1.0 may deliberately retain the warned precedence rule. This is a freeze-review input, not a preselected product requirement: a later consumer, compatibility analysis, or owner decision may show that the warning makes one path safely unsupported, that another additive disposition exists, or that retaining the current rule is preferable. No M4 app writes both | no — author surface plus pure focus logic; the eventual verdict may be no change | **none claimed** (owner, 2026-08-07); revisit with an actual combined-role consumer or the M6 public-surface review | [M4-Phase 2 T6 close gate CF-T6-2](./milestone-4/phase-2/implementation/log.md) (2026-08-07), owner-added |
