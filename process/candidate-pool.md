...
| Button-family content children — a `Button` / `ToggleButton` holds an authored subtree that layout arranges and hit-testing reaches, instead of today's label-only surface (a Button is a childless rectangle to layout, so a child written today is never arranged; M4-Phase 2 T8 turns that into a named diagnostic, and this item is what re-opens it deliberately) | no — `wasamo_widget_append_child` already admits any parent; the change is layout plus the admission rule | M5 widget set (a content-holding control is a widget-design decision — what becomes of the label, how children arrange, what the accessibility name is) | [M4-Phase 2 T3 close gate CF-1](./milestone-4/phase-2/implementation/log.md) (2026-08-07), owner-added |
