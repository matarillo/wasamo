...
and T13a closed with their separate task retrospective, then were merged
no-ff into this branch as `b42a212`. This record was created only after
that merge. The phase branch remains local and unpushed; actual GitHub
Actions CI and the phase-to-`main` merge are separate owner gates.
...
The `carry-forward` and named `doc-folded` entries above audit the closing
`implementation/handoff.md` prepared by T13. Closed items are not
reintroduced there as future work.
