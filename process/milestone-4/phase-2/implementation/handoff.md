...
| **CF-T7-1 / CF-T9-1** | Focus anchors are node addresses. T9's nearest same-drain free/allocate run did not reuse the address, so ABA remains possible but unobserved and bounded to unexpected focus, not memory unsafety. | Allocation-timing change around `mutate_for_loop_subtree`, or the existing fixture's collision arm firing. |
