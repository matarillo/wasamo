...
| 2026-07-29 | **[DD-M4-P1-003](./dd-m4-p1-003-dpi-change-propagation.md) annotated in two places**, body unchanged, on owner approval after the T4 independent review: §Context's "does not re-decide a single layout number" is qualified (exact invariance is a property of a controlled client extent — the non-client frame scales by its own DPI-indexed metrics, so the OS-suggested rectangle preserves the *outer* logical size and moves the client one), and option I1's "if the scale is not 1" is narrowed (the correction ships unconditional, because DD-M4-P1-001's tolerance of a failed declaration rests on the machinery having no second code path). **All statuses stay `Accepted` and nothing is superseded**: no decision, option or ordering changes, so a successor record would have no option to re-choose. This is the first post-acceptance annotation in the set; the shape follows [doc-system.md](../../../cross-milestone/decisions/doc-system.md)'s in-place annotation precedent — the original prose is left standing and the annotation points at the measurement. Evidence: [implementation/log.md](../implementation/log.md) §T4, findings F-28 / R-2 / R-3. |
