...
| **A host-package build does not rebuild the runtime** — a host reaches `wasamo.dll` through `wasamo-sys`'s build-script link-search path, not a cargo dependency edge, so `cargo build -p <host>` succeeds without recompiling `wasamo-runtime` and the launched host loads the previous DLL (T3 finding F-21; pre-existing, not introduced by this phase) | Every GUI evidence gate — T6, T9, T10 — and [AGENTS.md §Build ordering](../../../../AGENTS.md) at T12 | Any capture, smoke, or host run intended as evidence for a runtime change. Unlike F-5 this fails **silently and green**: the frame shows the old code and nothing reports an error. Precede every capture with `cargo build --release --workspace` |
