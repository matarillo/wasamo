...
| **Button-family widgets render no label without a layout pass**, and `lib.rs::window_add_widget` deliberately runs none — so that path shows a Button's background and not its label (T3, Codex review finding R-1) | Owner decision pending: document and keep / give it a layout pass / remove it | Any new attach path that puts a widget on screen **as content** without a layout pass. The label geometry now lives only in `sync_visuals`, so "attached" and "laid out" stopped being separable for Button-family widgets |
