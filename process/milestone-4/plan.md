...
| M4-Phase 2 — Event routing + focus model + generic click | ADR drafted (`Proposed`); owner review | — | [preamble.md](phase-2/decisions/preamble.md) | Milestone core. Framing owner-aligned 2026-08-05 (seven agreements, six of them explicitly as revisable hypotheses; touch = synthesized injection with stated limits, no hardware available). Pre-ADR spike discharged — [focus-traversal-spike.md](phase-2/decisions/exploration/focus-traversal-spike.md): traversal core as Win32-independent pure logic under unit test, plus a mechanism fixture driving it from a tree built through the real `.ui` → IR → runtime path. Five decisions drafted (target-then-bubble with no capture phase and consume-on-handle; layout-derived DIP hit rectangles with one topmost target, from which occlusion follows; one `FocusState` per window with tree-order traversal, group traversal and focus / active-item separation settled for M5 without building widgets; a modal scope as an annotated subtree plus an explicit entry, since the restore target is not derivable from the tree; per-item handlers admitted inside `for` bodies with their registration lifecycle and identity interaction; exactly two new authored annotations). The stretch re-evaluation checkpoint (§Cross-phase dispositions 3) fires at the Accepted flip |
