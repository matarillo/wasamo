...
| M4-Phase 2 — Event routing + focus model + generic click | DD-001…004 Accepted; **DD-005 `Proposed`** (key handling open, blocks implementation start) | — | [preamble.md](phase-2/decisions/preamble.md) | Milestone core. Framing owner-aligned 2026-08-05 (seven agreements, six of them explicitly as revisable hypotheses; touch = synthesized injection with stated limits, no hardware available). Pre-ADR spike discharged — [focus-traversal-spike.md](phase-2/decisions/exploration/focus-traversal-spike.md): traversal core as Win32-independent pure logic under unit test, plus a mechanism fixture driving it from a tree built through the real `.ui` → IR → runtime path. Five decisions Accepted 2026-08-05 (target-then-bubble with no capture phase and consume-on-handle; layout-derived DIP hit rectangles with one topmost target, from which occlusion follows rather than needing a scrim rule; one `FocusState` per window with tree-order traversal, group traversal and focus / active-item separation settled for M5 without building widgets; a modal scope as an annotated subtree plus an explicit entry, since the restore target is not derivable from the tree; per-item handlers admitted inside `for` bodies together with the registration lifecycle and the position-not-item identity consequence M3-Phase 7 routed here; exactly two new authored annotations). DD-005's key-handling half is reopened: K1 shipped no authored key surface, which leaves both Esc-closes and Left/Right without a mechanism and contradicts DD-004's "the act of closing is authored" — settled at implementation T0 by rewriting the section. **Stretch re-evaluation checkpoint is now due** (§Cross-phase dispositions 3) |
