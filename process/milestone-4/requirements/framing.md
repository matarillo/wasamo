...
本方針固めでオーナーが決めるのは、機能リストの確定（Q1）と題材アプリの
採択（Q2）。Q1 は互いに独立な 3 つの小問（Q1-1〜Q1-3）に分解して問う。
順序は依存方向どおり Q1 → Q2。各小問の詳細な根拠は §粒度の定義にあり、
ここでは「何を問われているか」を自己完結に要約する。

### Q1 — 機能リストの確定（提案あり、確認待ち）

§機能リストの表（P1〜P4）と §粒度の定義（提案）を確定する問い。実質の
判断は次の 3 つ。

#### Q1-1 — 文字入力の欄: 複数行の編集をどの重さで扱うか

- **ひとことで言うと**: メモ帳のように改行できる入力欄まで M4 で作る
  約束をするか、1 行だけの入力欄を約束の線にするか。
- **何が問題か**: AC は「最小の編集可能な入力欄（IME 検証に必要）」と
  しか言っておらず、「最小」の解釈で工数と題材アプリの選択肢が変わる。
  技術的には日本語入力（TSF）の検証に複数行は不要で、複数行を本筋として
  要求する候補は付箋アプリ（題材候補 B1）だけ。
- **選択肢**:
  1. **2 行に分けて置く（推奨）** — 単一行 = P1（AC 担当）、複数行 =
     P4（載れば取る）。買うもの: 事前の二者択一を避け、複数行を取るか
     どうかを Q2 のカバレッジ読みに委ねられる。諦めるもの: B1 を選ぶ
     場合に「P4 なのに落とせない」例外を引き受ける（表に注記済み）。
  2. **複数行も P1 に格上げ** — 買うもの: B1 が他候補と対等に競える。
     諦めるもの: AC が要求していない工数（上下移動・折返し・縦スクロール
     等）を採択前に無条件で確定させ、フォーカスモデル確定と並走する
     設計・検証面が跳ねる。
  3. **複数行を表から外して M5 送りを明記** — 買うもの: M4 が最も軽い。
     諦めるもの: B1 が即時脱落し、Q2 の選択肢が狭まる。
- 単一行の行の中身（カーソル・範囲選択・クリップボード・IME 結線・
  UIA。undo / 伏せ字 / 装飾は含めない）もこの小問で併せて確定する。
  詳細と技術的根拠は §粒度の定義（文字編集）と補足資料
  [single-line-text-editing.md](single-line-text-editing.md)。

#### Q1-2 — 重ね表示: 最前面に出す仕組みをどこまで作って証明するか

- **ひとことで言うと**: メニューやドロップダウンの土台になる「最前面の
  層」を、M4 でどの完成度まで作るか。
- **何が問題か**: この項目を中核に取り込んだ理由は「閉じ方・フォーカスの
  規則がフォーカスモデルと絡む」から。作りが浅すぎると M5 の部品実装で
  規則が通用せず主眼「フォーカスモデルの一回確定」が破れ、深すぎると
  部品の名前参照という文法の問い（M5 の領分）まで M4 に入り込む。
- **選択肢**（詳細比較表は §粒度の定義〔重ね表示〕）:
  1. **G1: M3 の重ね方のまま閉じ方の規則だけ足す** — 買うもの: 最安。
     諦めるもの: 最前面の層を一度も見ずにフォーカスモデルを確定するため、
     M5 での作り直しリスクを抱える。取り込み項目の実体も空になる。
  2. **G2: 最前面の層という構造まで作り、その上で規則一式を証明
     （推奨）** — 位置合わせは粗い手動。買うもの: フォーカスと絡む部分を
     すべて本物の構造の上で固める。文法の問いを開かない。諦めるもの:
     アプリのメニューの吸着精度（見本アプリの AC は仕上げの粗さを許容）。
  3. **G3: ボタン等に吸着して開く小窓まで作る** — 買うもの: アプリの
     書き味は最良。諦めるもの: 部品の名前参照という文法判断が M4 に
     入り、フォーカスモデル確定と並走する設計面が増える。

#### Q1-3 — 入力欄の値の書き戻しを、最初から中核に含めるか

- **ひとことで言うと**: 入力欄に打った文字をホスト側の状態へ自動で戻す
  仕組み（双方向束縛）を、単一行入力欄の最小構成に含めるか。
- **何が問題か**: 実質は既定路線の明文化。ホスト状態境界（中核取り込み）
  の「書き戻し」と、M3 から持ち越した双方向束縛の再開条件が、どちらも
  入力欄を指している。問いとして立てるのは、含めると入力欄の初回実装が
  ホスト状態境界の設計と同時進行になる（フェーズ順序を拘束する）ため。
- **選択肢**:
  1. **含める（推奨）** — 買うもの: 入力欄・ホスト状態境界・双方向束縛を
     一回の設計で整合させる。諦めるもの: 入力欄単体を先に軽く出す自由。
  2. **含めず後続フェーズへ分離** — 買うもの: 入力欄の初回実装が軽い。
     諦めるもの: 束縛の再開条件を二度扱うことになり、TSF の変更通知と
     ホスト書き込みの整合（両者の交差点）を後から再設計するリスク。

**Q1 確定後の処理**: §粒度の定義の「提案」表記を外して確定形に書き換え、
カバレッジの重み付け読み
（[target-app-wireframes.html](target-app-wireframes.html) — 文字編集の
行分割に合わせた行列の更新を含む）へ進む。
...
- **選択肢**: 4 案の比較は §題材アプリの選定と
  [target-app-wireframes.html](target-app-wireframes.html)。Q1 が推奨
  どおりに確定した場合の先読み: 付箋（B1）は複数行のコストを中核並みに
  引き受ける場合のみ残る / 設定画面（B2）はフォーカス・読み上げの検証
  密度が最強だが複数ウィンドウの証明を別建てする必要 / チャット（B3）は
  日本語入力とホスト状態境界が本筋で並行配列の圧力に注意 / ギャラリー
  （A）は工数最小・残務消化最強だが文字入力が周辺的。
...
1. オーナー: Q1 の確認（§オーナーへの問い、小問 Q1-1〜Q1-3）。
