...
機能リストの行のうち、粒度が自明でない 3 か所の確定内容（Q1、
2026-07-09 オーナー確定）。
...
- **検討ケース 1〜6 の分類** — 分類枠は検討ノートの規定どおり
  ...
  | 5. キーボードで閉じる / 移動 | Esc は重ね表示の確定粒度により M4 範囲。重ね表示内の Tab 移動はケース 4 と一体。矢印での前後移動はアプリ依存 | Esc / Tab は実質確定、矢印は採択後の改訂 |
...
- **ひとことで言うと**: M4 の機能を証明する見本アプリを、4 案（写真
  ギャラリー / 付箋 / 設定画面 / チャット）から 1 つ選ぶ。
- **何が問題か**: 実装する機能は同じでも、アプリによって「本筋として
  証明できる機能」が変わる（F4: 機能リストが先、アプリはそのカバレッジで
  選ぶ）。Q1 の答えで各案の評価が変わるため、Q1 の後に置く。
- **選択肢**: 4 案の比較は §題材アプリの選定と
  [target-app-wireframes.html](target-app-wireframes.html)。Q1 の確定を
  踏まえた先読み: 付箋（B1）は複数行のコストを中核並みに引き受ける場合
  のみ残る（この例外対応はオーナー受諾済み）/ 設定画面（B2）は
  フォーカス・読み上げの検証密度が最強だが複数ウィンドウの証明を別建て
  する必要 / チャット（B3）は日本語入力とホスト状態境界が本筋で並行配列
  の圧力に注意 / ギャラリー（A）は工数最小・残務消化最強だが文字入力が
  周辺的。
- **併せて問うもの**: 副次基準（工数・構造リスク・訴求力・残務消化）の
  重みの意向。
- **手続き**: 採択の明文化は `requirements/spec.md`（§1.2 の成果物）、
  採択に依存する帰結（§採択に依存して変わるもの）は本文書の改訂で
  確定する。
...
1. 確定した優先機能（Q1 確定済み）でカバレッジの重み付け読みを
   [target-app-wireframes.html](target-app-wireframes.html) に記録
   （文字編集の行分割に合わせた行列の更新を含む）→ オーナー: Q2
   題材アプリの採択（F5 確定）→ `requirements/spec.md` で明文化 +
   採択に依存する帰結を本文書の改訂で確定。
2. [candidate-pool.md](../../candidate-pool.md) の処遇記録に着地先リンク
   付きで記入（§1.1 時点の項目数 = 12 と、本方針固め中の追加 1 件
   〔吸着小窓〕を併記）。
3. ROADMAP M4 AC の改訂（中核取り込み 4 件の昇格 + アプリに照らした
   審査）。
