...
2. **その優先機能をどれだけ自然にカバーできるかで題材アプリを決める**
   ...
   題材アプリは 2 本立て（F6）: 写真ギャラリー成熟（題材候補 A、採択
   確定）+ 第二アプリ（B1 / B2 / B3 から選定、未決）。カバレッジは
   1 本で背負わず 2 本の組で満たす。
...
求める事項は §オーナーへの問い に集約**しており（Q1 は確定済み、未決は
Q2 のみ）、本文中の「Q1」「Q2」の記号はその節を指す。オーナーが今決める
必要のないものは §採択後・各フェーズへ送る決定事項に、着地先を固定して
送る。
...
本方針固めの前提となる決定の台帳。F1〜F4・F6 は合意済み、F5（第二
アプリ分）のみ未確定。
...
- **F4 — 機能リストが先、題材アプリはそのカバレッジで選ぶ**
  ...
  §題材アプリの選定。
- **F5 — 題材アプリの採択。一部確定**（→ §オーナーへの問い Q2）。
  A（写真ギャラリー成熟）の採択は F6 で確定。残る未確定は第二アプリ
  （B1 / B2 / B3）の選定のみ。採択の明文化は `requirements/spec.md`
  （§1.2 の成果物）。
...
| **P3** | 閉じ込め × 最前面層の組合せ（モーダルダイアログ相当。層は**予備** — §粒度の定義〔重ね表示〕） | F6 — 重ね表示とフォーカスモデルの組合せ実証 | 弱い差別化（3 候補とも自然な出番あり。文字入力を内蔵する改名 / 名前入力ダイアログが最も濃い） |
...
題材アプリは 2 本立て: **A（写真ギャラリー成熟）= 採択確定** +
**第二アプリ（B1 / B2 / B3 から選定、Q2 で未決）**。1 本のアプリに
...
- 単一行の文字編集 + IME（主眼の中心を本筋として証明する）。
- 複数ウィンドウ。
- 最前面の重ね表示（構造 + 外側クリックで閉じる型の規則）。
- ウィンドウ設定プロパティ（**動的タイトルを含む**。タイトル文字列の
  値の出どころ — 束縛かホスト合成か — は該当フェーズの ADR）。
- 閉じ込め × 最前面層の組合せ（モーダルダイアログ、予備層）。
- フォーカス / UIA 検証の密度（異種部品の Tab 順）。
- ホスト状態境界の書き戻しの方向（双方向束縛と一体）。
...
| 案 | 概要 | 一言でいうと |
|---|---|---|
| **A** | 写真ギャラリーの成熟（M3 継続） | **採択確定（F6）**。役割 = 自然な範囲 + M3 先送り事項の消化（§題材アプリ二本立てと役割分担） |
| **B1** | 付箋（Sticky Notes） | 複数ウィンドウ / らしさで最強、複数行の文字編集（P4 行）を事実上中核として要求 |
| **B2** | 設定画面の再現 | フォーカス / 重ね表示 / UIA の密度で最強、複数ウィンドウ・動的タイトルに穴 |
| **B3** | チャット（ローカルのおうむ返しボット） | IME / ホスト状態境界で最強、並行配列の圧力に注意 |

第二アプリ（B1 / B2 / B3 から 1 つ）は未決（Q2）。

### 第二アプリの採択に依存して変わるもの

第二アプリの採択で変わるのは次の 3 点:

- **B1（付箋）採択時** — 複数行の文字編集（P4 行）が事実上中核化する
  （§粒度の定義〔文字編集〕の注意）。`fill` の直値指定は付箋の色として
  第二アプリ側にも出番が生じる。
- **B2（設定画面）採択時** — 複数ウィンドウと動的タイトルに自然な
  出番がなく、証明の別建て（最小の検証用部品等）を追加で決める。
- **B3（チャット）採択時** — 並行配列の圧力点検（`TypedValue` の宿題、
  §検討ノートのケース分類と宿題）を前倒しする。
...
- **検討ケース 1〜6 の分類** — 分類枠は検討ノートの規定どおり
  ...
  | ケース | 現状の見込み | 確定の時期 |
  |---|---|---|
  | 1. クリックで開く（サムネイル等） | ケース 3 の帰結 + A で確定（サムネイルクリック → ライトボックス、F6） | 実質確定（記入は第二アプリ採択後の改訂で一括） |
  | 2. 外側クリックで閉じる | 重ね表示の確定粒度により M4 範囲（アプリ非依存、§粒度の定義〔重ね表示〕） | 実質確定（記入は採択後の改訂で一括） |
  | 3. Button 以外への汎用クリック処理 | **全候補に共通**（一覧の行クリックはどの候補にもある）。機能リスト P1 行（汎用クリック処理）として実装、設計は §粒度の定義（フォーカスモデル） | イベント配送フェーズの ADR |
  | 4. フォーカス閉じ込め | 重ね表示の確定粒度により M4 範囲（アプリ非依存） | 実質確定（同上） |
  | 5. キーボードで閉じる / 移動 | Esc は重ね表示の確定粒度により M4 範囲。重ね表示内の Tab 移動はケース 4 と一体。矢印での前後移動は A のライトボックス ← → で確定（F6） | 実質確定（記入は第二アプリ採択後の改訂で一括） |
...
本方針固めでオーナーの決定を要するのは、機能リストの確定（Q1、完了）と
第二アプリの採択（Q2、未決。A の採択は F6 で確定済み）。
...
### Q2 — 第二アプリの採択（= F5 の残余、未確定）

- **ひとことで言うと**: A（写真ギャラリー成熟）とペアで M4 の機能を
  証明する第二アプリを、付箋（B1）/ 設定画面（B2）/ チャット（B3）から
  1 つ選ぶ。
- **何が問題か**: A の役割確定（F6）により、第二アプリが担うべきものは
  §題材アプリ二本立てと役割分担のとおり、単一行の文字編集 + IME・
  複数ウィンドウ・最前面の重ね表示・ウィンドウ設定プロパティ（動的
  タイトル）・閉じ込め × 最前面層（モーダルダイアログ）・フォーカス /
  UIA 検証の密度・ホスト状態境界の書き戻しの方向に絞られた。この担当分を
  本筋としてどれだけ自然にカバーできるかで選ぶ。
- **現時点の読み**（重み付け読みの記録は
  [target-app-wireframes.html](target-app-wireframes.html)）:
  チャット（B3）は担当分の全項目に自然な出番があり穴がない（注意は
  並行配列の圧力のみ）/ 設定画面（B2）はフォーカス・読み上げの検証密度が
  最強だが複数ウィンドウ・動的タイトルの 2 つの穴に証明の別建てが要る /
  付箋（B1）は複数ウィンドウの密度が最強だが複数行の文字編集のコストを
  中核並みに引き受ける判断とセット（この例外対応はオーナー受諾済み）。
- **併せて問うもの**: 副次基準（工数・構造リスク・訴求力・残務消化）の
  重みの意向。残務消化は A が確定で担うため、実質は工数・構造リスク・
  訴求力の 3 つ。
- **手続き**: 採択の明文化は `requirements/spec.md`（§1.2 の成果物）、
  採択に依存する帰結（§第二アプリの採択に依存して変わるもの）は
  本文書の改訂で確定する。
...
3. **複数ウィンドウ・動的タイトルの証明の形** — 設定画面案（題材候補
   B2）を第二アプリに採択した時のみ。アプリ内での証明か、最小の検証用
   部品の別建てか。採択後の spec.md で決める。
4. **サイズ試掘の置き場** — どのフェーズに置くか（既定は M4 実施。
   M5 送りにするなら VDR の求める積極的な正当化を記録）。§1.4 plan.md
   で確定する。

## 次の一手

1. 第二アプリの担当分での重み付け読みは
   [target-app-wireframes.html](target-app-wireframes.html) に記録済み →
   オーナー: Q2 第二アプリの採択（F5 の残余確定）→
   `requirements/spec.md` で 2 本の明文化 + 採択に依存する帰結を本文書の
   改訂で確定。
2. [candidate-pool.md](../../candidate-pool.md) の処遇記録に着地先リンク
   付きで記入（§1.1 時点の項目数 = 12 と、本方針固め中の追加 1 件
   〔吸着小窓〕を併記）。
3. ROADMAP M4 AC の改訂（中核取り込み 4 件の昇格 + 2 本のアプリに
   照らした審査 + 見本アプリ AC の単数前提の審査）。
